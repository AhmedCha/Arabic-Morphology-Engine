...
inline void morphologyMenu(AVLTree<string>& tree, SchemeHashTable& schemes) {
    ...
    while (true) {
        ...
        cout << "1. Generate Word (Root + Scheme)" << endl;
        cout << "2. Validate Word (Word + Specific Root)" << endl;
        cout << "3. Detect Root (Word -> Search Database)" << endl;
        cout << "0. Back to Main Menu" << endl;
        cout << "Choice: ";
        cin >> choice;
        ...
        switch (choice) {
            case 1: // GENERATE
                ...
                cout << "Enter Scheme Name: "; cin >> schemeName;
                {
                    ...
                    if (pattern == "") cout << "Scheme not found." << endl;
                    else cout << "-> Result: " << MorphologyEngine::generate(root, pattern) << endl;
                }
                break;
            ...
            case 2: // VALIDATE
                ...
                {
                    string foundScheme;
                    if (MorphologyEngine::validate(word, root, schemes, foundScheme))
                        cout << "✔ VALID (Matches scheme: " << foundScheme << ")" << endl;
                    else
                        cout << "✘ INVALID (Does not match root with known schemes)" << endl;
                }
                break;
